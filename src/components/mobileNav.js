...
function AddMenu(name, url) {
  return (
    <Link
      className="block md:hidden mt-4 md:mt-0 md:ml-3 no-underline font-bold text-white"
      key={name}
      to={url}
    >
      {name}
...
function AddSubMenu(name, url) {
  return (
    <Link
      className="block md:hidden pl-4 mt-4 no-underline text-white"
      key={name}
      to={url}
    >
      {name}
  ...
  return (
    <MobileView>
        ...
        >
          ...
          {AddMenu("Members", "/members")}
          {membersMenu.map((node,index) => {
                      return (
